...
/* THIS FILE : /home/paslab/CLOC/examples/snack/test_snack/hw.snackwrap.c  */
/* INPUT FILE: /home/paslab/CLOC/examples/snack/test_snack/hw.cl  */
/* UPDATED CL: /tmp/snk_13450/updated.cl  */
/*                               */ 
...
hsa_ext_program_t                hw_HsaProgram;
hsa_executable_t                 hw_Executable;
hsa_region_t                     hw_KernargRegion;
int                              hw_FC = 0; 
...
status_t hw_InitContext(){
    ...
    err = hsa_iterate_agents(get_gpu_agent, &hw_Agent);
    if(err == HSA_STATUS_INFO_BREAK) { err = HSA_STATUS_SUCCESS; }
    ErrorCheck(Getting a gpu agent, err);

    /* Query the name of the agent.  */
    char name[64] = { 0 };
    err = hsa_agent_get_info(hw_Agent, HSA_AGENT_INFO_NAME, name);
    ErrorCheck(Querying the agent name, err);
    /* printf("The agent name is %s.\n", name); */

    /* Query the maximum size of the queue.  */
    uint32_t queue_size = 0;
    err = hsa_agent_get_info(hw_Agent, HSA_AGENT_INFO_QUEUE_MAX_SIZE, &queue_size);
    ErrorCheck(Querying the agent maximum queue size, err);
    /* printf("The maximum queue size is %u.\n", (unsigned int) queue_size);  */

    /* Create hsa program.  */
    memset(&hw_HsaProgram,0,sizeof(hsa_ext_program_t));
    err = hsa_ext_program_create(HSA_MACHINE_MODEL_LARGE, HSA_PROFILE_FULL, HSA_DEFAULT_FLOAT_ROUNDING_MODE_DEFAULT, NULL, &hw_HsaProgram);
    ErrorCheck(Create the program, err);

    /* Add the BRIG module to hsa program.  */
    err = hsa_ext_program_add_module(hw_HsaProgram, (hsa_ext_module_t) hw_HSA_BrigMem );
    ErrorCheck(Adding the brig module to the program, err);

    /* Determine the agents ISA.  */
    hsa_isa_t isa;
    err = hsa_agent_get_info(hw_Agent, HSA_AGENT_INFO_ISA, &isa);
    ...
    err = hsa_ext_program_finalize(hw_HsaProgram, isa, 0, control_directives, "", HSA_CODE_OBJECT_TYPE_PROGRAM, &code_object);
    ErrorCheck(Finalizing the program, err);

    /* Destroy the program, it is no longer needed.  */
    err=hsa_ext_program_destroy(hw_HsaProgram);
    ErrorCheck(Destroying the program, err);

    /* Create the empty executable.  */
    err = hsa_executable_create(HSA_PROFILE_FULL, HSA_EXECUTABLE_STATE_UNFROZEN, "", &hw_Executable);
    ErrorCheck(Create the executable, err);

    /* Load the code object.  */
    err = hsa_executable_load_code_object(hw_Executable, hw_Agent, code_object, "");
    ErrorCheck(Loading the code object, err);

    /* Freeze the executable; it can now be queried for symbols.  */
    err = hsa_executable_freeze(hw_Executable, "");
    ErrorCheck(Freeze the executable, err);

    /* Find a memory region that supports kernel arguments.  */
    hw_KernargRegion.handle=(uint64_t)-1;
    hsa_agent_iterate_regions(hw_Agent, get_kernarg_memory_region, &hw_KernargRegion);
    err = (hw_KernargRegion.handle == (uint64_t)-1) ? HSA_STATUS_ERROR : HSA_STATUS_SUCCESS;
    ErrorCheck(Finding a kernarg memory region, err);

    /*  Create a queue using the maximum size.  */
    err = hsa_queue_create(hw_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &Sync_CommandQ);
    ...
    for ( stream_num = 0 ; stream_num < SNK_MAX_STREAMS ; stream_num++){
       ...
       err=hsa_queue_create(hw_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE, NULL, NULL, UINT32_MAX, UINT32_MAX, &Stream_CommandQ[stream_num]);
...
hsa_executable_symbol_t          cal_diskernel_Symbol;
int                              cal_diskernel_FK = 0 ; 
status_t                         cal_diskernel_init();
status_t                         cal_diskernel_stop();
uint64_t                         cal_diskernel_Kernel_Object;
uint32_t                         cal_diskernel_Kernarg_Segment_Size; /* May not need to be global */
uint32_t                         cal_diskernel_Group_Segment_Size;
uint32_t                         cal_diskernel_Private_Segment_Size;

/* ------  Start of SNACK function cal_diskernel ------ */ 
extern void cal_diskernel(float* data_ker,float* cent_ker,int* table,int  K,int  DIM,int  DCNT,int* chpt,int* cent_c_ker,float* min_dis, const snk_lparm_t * lparm) {
   /* Kernel initialization has to be done before kernel arguments are set/inspected */ 
   if (cal_diskernel_FK == 0 ) { 
     status_t status = cal_diskernel_init(); 
   ...
   thisKernargAddress = malloc(cal_diskernel_Kernarg_Segment_Size); 
   struct cal_diskernel_args_struct {
      uint64_t arg0;
      uint64_t arg1;
      uint64_t arg2;
      uint64_t arg3;
   ...
   struct cal_diskernel_args_struct* cal_diskernel_args ; 
   /* Setup kernel args */ 
   cal_diskernel_args = (struct cal_diskernel_args_struct*) thisKernargAddress; 
   cal_diskernel_args->arg0=0 ; 
   cal_diskernel_args->arg1=0 ; 
   cal_diskernel_args->arg2=0 ; 
   cal_diskernel_args->arg3=0 ; 
   cal_diskernel_args->arg4=0 ; 
   cal_diskernel_args->arg5=0 ; 
   cal_diskernel_args->arg6 = data_ker ; 
   cal_diskernel_args->arg7 = cent_ker ; 
   cal_diskernel_args->arg8 = table ; 
   cal_diskernel_args->arg9 = K ; 
   cal_diskernel_args->arg10 = DIM ; 
   cal_diskernel_args->arg11 = DCNT ; 
   cal_diskernel_args->arg12 = chpt ; 
   cal_diskernel_args->arg13 = cent_c_ker ; 
   cal_diskernel_args->arg14 = min_dis ; 
    ...
    this_aql->kernel_object = cal_diskernel_Kernel_Object;
    this_aql->private_segment_size = cal_diskernel_Private_Segment_Size;
    this_aql->group_segment_size = cal_diskernel_Group_Segment_Size;
...
/* ------  End of SNACK function cal_diskernel ------ */ 
extern status_t cal_diskernel_init(){

    if (hw_FC == 0 ) {
       status_t status = hw_InitContext();
       if ( status  != STATUS_SUCCESS ) return; 
       hw_FC = 1;
    }
    ...
    /* printf("Kernel name cal_diskernel_: Looking for symbol %s\n","__OpenCL_cal_diskernel_kernel"); */
    err = hsa_executable_get_symbol(hw_Executable, NULL, "&__OpenCL_cal_diskernel_kernel", hw_Agent , 0, &cal_diskernel_Symbol);
    ErrorCheck(Extract the symbol from the executable, err);

    /* Extract dispatch information from the symbol */
    err = hsa_executable_symbol_get_info(cal_diskernel_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT, &cal_diskernel_Kernel_Object);
    ErrorCheck(Extracting the symbol from the executable, err);
    err = hsa_executable_symbol_get_info(cal_diskernel_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE, &cal_diskernel_Kernarg_Segment_Size);
    ErrorCheck(Extracting the kernarg segment size from the executable, err);
    err = hsa_executable_symbol_get_info(cal_diskernel_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE, &cal_diskernel_Group_Segment_Size);
    ErrorCheck(Extracting the group segment size from the executable, err);
    err = hsa_executable_symbol_get_info(cal_diskernel_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE, &cal_diskernel_Private_Segment_Size);
    ErrorCheck(Extracting the private segment from the executable, err);


    return STATUS_SUCCESS;

} /* end of cal_diskernel_init */


extern status_t cal_diskernel_stop(){
    status_t err;
    if (hw_FC == 0 ) {
